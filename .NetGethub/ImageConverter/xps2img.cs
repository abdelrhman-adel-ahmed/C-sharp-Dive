...
namespace xps2img
{
    ...
    public class Xps2Image : IDisposable
    {
        ...
        private Bitmap ProcessPage(Parameters parameters, int docPageNumber)
        {
            ...
            //problem here if we dispose the mem stream , question here why it work with other formates
            var stream = new MemoryStream();

            bitmapEncoder.Frames.Add(BitmapFrame.Create(bitmapSource));
            bitmapEncoder.Save(stream);
            string outputFileName = Path.Combine("D:\\xsp" + "\\Dsa" + ".Jpeg");
            ...
            try
            {
                using (var page = documentPaginator.GetPage(pageNumber))
                {
                    ...
                    var bitmap = new RenderTargetBitmap((int)Math.Round(page.Size.Width * ratio),
                                                        (int)Math.Round(page.Size.Height * ratio), dpi, dpi, PixelFormats.Default);
